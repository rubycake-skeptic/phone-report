<template>
    <v-card outlined>
        ...
        <v-card-actions>
            ...
            <v-dialog v-model="addDeviceDiagram" width="500">
                <AddDeviceCommand
                    @closeDialog="closeAddDevice"
                    @addDevice="addDevice"
                ></AddDeviceCommand>
            </v-dialog>
        </v-card-actions>
	...
	async created() {
        ...
        methods: {
            ...
            async addDevice() {
                try {
                    if(!this.offline){
                        var temp = await axios.post(axios.fixUrl(this.value._links['/adddevice'].href))
                        for(var k in temp.data) this.value[k]=temp.data[k];
                    }
